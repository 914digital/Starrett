...
if ( ! function_exists( 'woo_google_maps' ) ) {
	...
	function woo_google_maps() {
		...
		if ( is_page_template( 'template-contact.php' ) ) {
			...
			if ( is_ssl() ) {
				$maps_url = str_replace( 'http://', 'https://', $maps_url );
			}
...
if ( ! function_exists( 'woo_custom_styling' ) ) {
	...
	function woo_custom_styling() {
		...
		if ( $font_post_title ) {
			$output .= '.page-title, .post .title, .page .title {' . woo_generate_font_css( $font_post_title,
					1.1 ) . '}' . "\n";
		}
		...
		if ( $widget_font_text ) {
			$output .= '.widget p, .widget .textwidget { ' . woo_generate_font_css( $widget_font_text,
					1.5 ) . ' }' . "\n";
		}
		...
		if ( $widget_tabs_font ) {
			$output .= '#tabs .inside li a, .widget_woodojo_tabs .tabbable .tab-pane li a { ' . woo_generate_font_css( $widget_tabs_font,
					1.5 ) . ' }' . "\n";
		}

		if ( $widget_tabs_font_meta ) {
			$output .= '#tabs .inside li span.meta, .widget_woodojo_tabs .tabbable .tab-pane li span.meta { ' . woo_generate_font_css( $widget_tabs_font_meta,
					1.5 ) . ' }' . "\n";
		}

		$output .= '#tabs ul.wooTabs li a, .widget_woodojo_tabs .tabbable .nav-tabs li a { ' . woo_generate_font_css( $widget_tabs_font_meta,
				2 ) . ' }' . "\n";
		...
		if ( $nav_font ) {
			$output .= 'ul.nav li a, #navigation ul.rss a, #navigation .cart-contents, #navigation ul.rss, #navigation ul.cart .cart_list a{ ' . woo_generate_font_css( $nav_font,
					1.2 ) . ' } #navigation ul.rss li a:before { color:' . $nav_font['color'] . ';}' . "\n";
		}
		...
		if ( $slider_magazine_font_excerpt ) {
			$output .= '.magazine #loopedSlider .content .excerpt p { ' . woo_generate_font_css( $slider_magazine_font_excerpt,
					1.5 ) . ' }' . "\n";
		}

		if ( $magazine_grid_font_post_title ) {
			$output .= '.magazine .block .post .title, .magazine .block .post .title a { ' . woo_generate_font_css( $magazine_grid_font_post_title,
					1.2 ) . ' }' . "\n";
		}
		...
		if ( $slider_biz_font_excerpt ) {
			$output .= '#wrapper #loopedSlider.business-slider .content p { ' . woo_generate_font_css( $slider_biz_font_excerpt,
					1.5 ) . ' }' . "\n";
		}
	...
	function woo_slider( $load_slider_js = false ) {
		...
		if ( $load_slider_js != false ) {
			...
			if ( isset( $woo_options['woo_slider_pagination'] ) && $woo_options['woo_slider_pagination'] == 'true' ) {
				$args['generatePagination'] = 'true';
			}

			if ( isset( $woo_options['woo_slider_effect'] ) && $woo_options['woo_slider_effect'] != '' ) {
				$args['effect'] = $woo_options['woo_slider_effect'];
			}
			...
			if ( isset( $woo_options['woo_slider_hover'] ) && $woo_options['woo_slider_hover'] == 'true' ) {
				$args['hoverPause'] = 'true';
			}

			if ( isset( $woo_options['woo_slider_containerclick'] ) && $woo_options['woo_slider_containerclick'] == 'true' ) {
				$args['containerClick'] = 'true';
			}

			if ( isset( $woo_options['woo_slider_speed'] ) && $woo_options['woo_slider_speed'] != '' ) {
				$args['slideSpeed'] = $woo_options['woo_slider_speed'] * 1000;
			}

			if ( isset( $woo_options['woo_slider_speed'] ) && $woo_options['woo_slider_speed'] != '' ) {
				$args['fadeSpeed'] = $woo_options['woo_slider_speed'] * 1000;
			}
			...
			<script type="text/javascript">
				...
				jQuery(window).load(function () {
					...
					<?php } else { ?>args.slideshow = false;
					<?php } ?>

					<?php echo "\n"; ?>

					<?php if ( intval( $args['autoStart'] ) > 0 ) { ?>args.slideshowSpeed = <?php echo intval( $args['autoStart'] ) ?>;
					<?php } ?>

					<?php echo "\n"; ?>

					<?php if ( intval( $args['slideSpeed'] ) >= 0 ) { ?>args.animationSpeed = <?php echo intval( $args['slideSpeed'] ) ?>;
					<?php } ?>
					...
					<?php } else { ?>args.controlNav = false;
					<?php } ?>
					...
					<?php } else { ?>args.pauseOnHover = false;
					<?php } ?>

					<?php echo "\n"; ?>

					<?php if ( $args['autoHeight'] == 'true' ) { ?>args.smoothHeight = true;
					<?php } ?>
...
if ( ! function_exists( 'woo_slider_magazine' ) ) {
	...
	function woo_slider_magazine( $args = null, $tags = null ) {
		...
		if ( empty( $tag_array ) ) {
			...
			echo do_shortcode( '[box type="alert"]' . __( 'Setup slider by adding <strong>Post Tags</strong> in <em>Magazine Template > Posts Slider</em>.',
					'woothemes' ) . '[/box]' );
...
if ( ! function_exists( 'woo_slider_get_slides' ) ) {
	...
	function woo_slider_get_slides( $args ) {
		...
		if ( false != $args['use_slide_page'] ) {
			...
			if ( is_numeric( $args['slide_page_terms'] ) ) {
				$slide_type = 'id';
			}
...
if ( ! function_exists( 'woo_slider_biz' ) ) {
	...
	function woo_slider_biz( $args = null ) {
		...
		if ( false == $slides ) {
			...
			echo do_shortcode( '[box type="alert"]' . __( 'Please add some slider posts via Slides > Add New',
					'woothemes' ) . '[/box]' );
		...
		if ( ( count( $slides ) < 1 ) ) {
			...
			echo do_shortcode( '[box type="alert"]' . __( 'Please note that this slider requires 2 or more slides in order to function. Please add another slide.',
					'woothemes' ) . '[/box]' );
...
if ( ! function_exists( 'woo_slider_biz_view' ) ) {
	...
	function woo_slider_biz_view( $args = null, $slides = null ) {
		...
		<div
			id="<?php echo esc_attr( $args['id'] ); ?>"<?php if ( '' != $args['container_css'] ): ?> class="<?php echo esc_attr( $args['container_css'] ); ?>"<?php endif; ?><?php if ( '' != $args['height'] ): ?> style="height: <?php echo esc_attr( $args['height'] ); ?>px;"<?php endif; ?>>


			<ul class="slides"<?php if ( '' != $args['height'] ): ?> style="height: <?php echo $args['height']; ?>px;"<?php endif; ?>>


				<?php foreach ( $slides as $k => $post ) {
					setup_postdata( $post );
					$count ++; ?>
					...
					if ( $count >= 2 ) {
						$args['slide_styles'] .= ' display:none;';
					} else {
						$args['slide_styles'] = '';
					}
					...
					<li id="slide-<?php echo esc_attr( $post->ID ); ?>"
					    class="slide slide-number-<?php echo esc_attr( $count ); ?>"
					    <?php if ( '' != $args['slide_styles'] ): ?>style="<?php echo esc_attr( $args['slide_styles'] ); ?>"<?php endif; ?>>
								...
								<?php if ( 'true' == $woo_options['woo_slider_biz_title'] ): ?>
									...
									<div class="title">
										...
										<h2 class="title">
											...
											<?php if ( '' != $url ): ?><a href="<?php echo esc_url( $url ); ?>"
											                              title="<?php the_title_attribute(); ?>"><?php endif; ?>
		...
		if ( ( isset( $woo_options['woo_nav_rss'] ) ) && ( $woo_options['woo_nav_rss'] == 'true' ) || ( isset( $woo_options['woo_subscribe_email'] ) ) && ( $woo_options['woo_subscribe_email'] ) ) { ?>
			...
			<ul class="rss fr<?php echo $class; ?>">
				...
				<?php if ( ( isset( $woo_options['woo_subscribe_email'] ) ) && ( $woo_options['woo_subscribe_email'] ) ) { ?>
					...
					<li class="sub-email"><a href="<?php echo esc_url( $woo_options['woo_subscribe_email'] ); ?>"></a>
					</li>
				...
				<?php if ( isset( $woo_options['woo_nav_rss'] ) && ( $woo_options['woo_nav_rss'] == 'true' ) ) { ?>
					...
					<li class="sub-rss"><a href="<?php if ( $woo_options['woo_feed_url'] ) {
							echo esc_url( $woo_options['woo_feed_url'] );
						} else {
							echo esc_url( get_bloginfo_rss( 'rss2_url' ) );
						} ?>"></a></li>
	...
	function woo_author_box() {
		...
		<aside id="post-author">
			...
			<div class="profile-content">
				...
				<h4><?php printf( esc_attr__( 'About %s', 'woothemes' ),
						get_the_author_meta( 'display_name', $author_id ) ); ?></h4>
					...
					<div class="profile-link">
						...
						<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID',
							...
							<?php printf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>',
								'woothemes' ), get_the_author_meta( 'display_name', $author_id ) ); ?>
...
if ( ! function_exists( 'woo_subscribe_connect_action' ) ) {
	...
	function woo_subscribe_connect_action() {
		...
		if ( is_single() && 'true' == get_option( 'woo_connect' ) ) {
...
if ( ! function_exists( 'woo_footer_left' ) ) {
	...
	function woo_footer_left() {
		...
		$settings = woo_get_dynamic_values( array(
			'footer_left'      => 'true',
			'footer_left_text' => '[site_copyright]'
		) );
...
if ( ! function_exists( 'woo_post_meta' ) ) {
	...
	function woo_post_meta() {
		...
		$post_info = '<span class="small">' . __( 'By',
				'woothemes' ) . '</span> [post_author_posts_link] <span class="small">' . _x( 'on', 'post datetime',
				'woothemes' ) . '</span> [post_date] <span class="small">' . __( 'in',
				'woothemes' ) . '</span> [post_categories before=""] ';
...
if ( ! function_exists( 'woo_display_post_image' ) ) {
	...
	function woo_display_post_image() {
		...
		$options = woo_get_dynamic_values( array(
			'thumb_w'            => '100',
			'thumb_h'            => '100',
			'thumb_align'        => 'alignleft',
			'single_w'           => '100',
			'single_h'           => '100',
			'thumb_align_single' => 'alignright',
			'thumb_single'       => 'false'
		) );
	...
	function woo_comment_form_fields( $fields ) {
		...
		$fields = array(
			...
			'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" class="txt" tabindex="1" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' />' .
					...
					'woothemes' ) . ( $req ? ' <span class="required">(' . __( 'required',
						'woothemes' ) . ')</span>' : '' ) . '</label> ' . '</p>',
			'email'  => '<p class="comment-form-email"><input id="email" name="email" type="text" class="txt" tabindex="2" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' />' .
			            ...
			            '<label for="email">' . __( 'Email (will not be published)',
					'woothemes' ) . ( $req ? ' <span class="required">(' . __( 'required',
						'woothemes' ) . ')</span>' : '' ) . '</label> ' . '</p>',
...
if ( ! function_exists( 'woo_comment_form_args' ) ) {
	...
	function woo_comment_form_args( $args ) {
		...
		$args['comment_field'] = str_replace( '<textarea ', '<textarea tabindex="' . $tabindex . '" ',
			$args['comment_field'] );
...
if ( ! function_exists( 'woo_feedburner_link' ) ) {
	...
	function woo_feedburner_link( $output, $feed = null ) {
		...
		if ( isset( $woo_options['woo_feed_url'] ) && $woo_options['woo_feed_url'] && ( $feed == $default ) && ( ! stristr( $output,
				'comments' ) )
		) {
			$output = $woo_options['woo_feed_url'];
		}
...
if ( ! function_exists( 'woo_load_site_width_css' ) ) {
	...
	function woo_load_site_width_css() {
		...
		if ( 0 < $layout_width && 960 != $layout_width ) { /* Has legitimate width */
		} else {
			return;
		} // Use default width from stylesheet
		...
		<style type="text/css">
			...
			.col-full, #wrapper {
				max-width: <?php echo intval( $layout_width ); ?>px !important;
			}
...
if ( ! function_exists( 'woo_load_site_width_css_nomedia' ) ) {
	...
	function woo_load_site_width_css_nomedia() {
		...
		if ( 0 < $layout_width ) { /* Has legitimate width */
		} else {
			$layout_width = 960;
		} // Default Width
		...
		<style type="text/css">.col-full, #wrapper {
				width: <?php echo intval( $layout_width ); ?>px;
				max-width: <?php echo intval( $layout_width ); ?>px;
			}
			...
			body.fixed-mobile #top, body.fixed-mobile #header-container, body.fixed-mobile #footer-container, body.fixed-mobile #nav-container, body.fixed-mobile #footer-widgets-container {
				min-width: <?php echo intval( $layout_width ); ?>px;
				padding: 0 1em;
			}
...
if ( ! function_exists( 'woo_modify_magazine_homepage_query' ) ) {
	...
	function woo_modify_magazine_homepage_query( $q ) {
		...
		if ( ! is_admin() && $q->is_main_query() && ( 0 < $q->query_vars['page_id'] ) && ( $q->query_vars['page_id'] == get_option( 'page_on_front' ) ) && ( 'template-magazine.php' == get_post_meta( intval( $q->query_vars['page_id'] ),
...
if ( ! function_exists( 'woo_add_full_width_class' ) ) {
	...
	function woo_add_full_width_class( $classes ) {
		...
		$settings = woo_get_dynamic_values( array(
			...
			'layout_boxed'      => '',
			'slider_biz_full'   => 'false'
		) );

		if ( 'true' == $settings['layout_boxed'] ) {
			return $classes;
		} // Don't add the full width CSS classes if boxed layout is enabled.
...
if ( ! function_exists( 'woo_portfolio_page_content' ) ) {
	...
	function woo_portfolio_page_content() {
		...
		if ( have_posts() ) {
			...
			woo_get_template_part( 'content',
				'page-template-business' ); // Use business content so we don't output a page title
